...
// 타석예약 불가
function used() {
  $('.tab-box__tab-cont > div .floor__row .floor__row__col').each(function () {
...
$(document).on('click', '.white_box.result_box ul li', function () {
  // purchase_box 활성화
  $('.gray_box.purchase_box.one').addClass('on');

  // purchase_box 닫기
  $('.gray_box.purchase_box.two').removeClass('on');
});

// 취소 클릭
$(document).on('click', '.btn_transfer_cancel', function (e) {
  e.preventDefault();

  // purchase_box 닫기
  $('.gray_box.purchase_box.one').removeClass('on');
});

// 양도 클릭
$(document).on('click', '.btn_transfer', function (e) {
  e.preventDefault();
