...
<%@ include file="/include/jsp/globals.jsp"%>
<!DOCTYPE html>
<html lang="ko">
    <head>
        <meta charset="UTF-8" />
        ...
        <div class="wrap">
            ...
            <header>
                ...
                <div class="right">
                  <a href="/app/myPage.do">
		            <img src="/resources/img/assets/ico_setting_white.svg" alt="설정아이콘" />
		          </a>
                </div>
            </header>
            <!-- navigation bar -->
	        <nav class="navigation-bar">
	          <ul class="navigation-bar__list">
	              ...
	              <li>
	                   <a href="javascript:serviceAlert('서비스 준비 중입니다.')">
	          <!-- <a href="product.do"> -->
	                      <span>
	                          <img
            ...
            <div class="container">
                <div class="search-box search-date">
                   <label>
						  ...
						  <c:choose>
						    <c:when test="${not empty salesList}">
						      <c:forEach var="sale" items="${salesList}">
						        <option value="${sale.SalesIndex}">${sale.MembershipName}</option>
						      </c:forEach>
						      <option value="all">전체</option>
						    </c:when>
						    <c:otherwise>
						      <option value="">이용중인 상품이 없습니다.</option>
						      <option value="all">전체</option>
						    </c:otherwise>
						  </c:choose>
						</select>
			...
			<div id="infoModal" class="custom-modal" style="display: none;">
			  <div class="custom-modal-content">
			    <div class="custom-modal-header">
			      ...
			      <button class="custom-close-btn" onclick="closeModal('infoModal')"></button>
			    </div>
			    <div class="custom-modal-body" id="infoModalMessage" style="text-align: center; font-size:1.6rem;">
	     
	    		</div>
	    		<div class="custom-modal-footer" style="text-align: center; margin-top: 2.5rem;">
				  <button id="infoModalOk" class="signup__cont__btn-box_2">확인</button>
				</div>
			  </div>
			</div>
        </div>
    ...
    <script>
	    document.addEventListener("DOMContentLoaded", function () {
	        ...
	        document.getElementById("startDt").value = formatDate(threeMonthsAgo);
	    ...
	    function onStartDtChange() {
	    	var startDtInput = document.querySelector("input[name='startDt']");
	        var endDtInput = document.querySelector("input[name='endDt']");
	    	var salesIndex = document.getElementById("product-select").value;  // 선택되어있는 상품코드
    	   	var data = { 
    	    ...
    	    $.ajax({
    	        ...
    	        success: function(response) {
    	            ...
    	            if (bookingList.length === 0) {
    	                listContainer.append('<li>조회된 결과가 없습니다.</li>');
    	                return;
    	            }
    	            ...
    	            bookingList.forEach(function(list, index) {
    	                ...
    	                if (list.SalesIndex !== prevSalesIndex) {
    	                    ...
    	                    var headerItem = '<p>' + list.MembershipName + '<span>' + status + '</span></p>';
    	                ...
    	                if (list.bookingStatus === '예약취소') {
    	                    bookingStatusText = ' <span style="color:red; font-size:1.2rem; margin-left:0.6rem;">취소</span>';
    	                } else if (list.bookingStatus === '예약중') {
    	                    bookingStatusText = ' <span style="color:green; font-size:1.2rem; margin-left:0.6rem;">예약중</span>';
    	                } else if (list.bookingStatus === '사용완료') {
    	                    bookingStatusText = ' <span style="color:blue; font-size:1.2rem; margin-left:0.6rem;">사용완료</span>';
    	                } else if (list.bookingStatus === '이용중') {
    	                    bookingStatusText = ' <span style="color:blue; font-size:1.2rem; margin-left:0.6rem;">이용중</span>';
    	                }

    	                // ✅ 예약 시간 표시: StartTimestamp 없으면 BookingCheckInDatetime 사용
    	                var startTime = list.StartTimestamp || list.BookingCheckInTime;
    	                var endTime = list.CheckOutTimestamp || list.BookingCheckoutDatetime;

    	                var timeParts = formatDateTime(startTime, endTime);
    	                var bayName = list.BayName || '0';
    	                var bayNameOnlyNumber = bayName.match(/\d+/) ? bayName.match(/\d+/)[0] : bayName;

    	                var bayDisplay = (bayNameOnlyNumber !== '0' && bayNameOnlyNumber !== '') 
    	                ? '<span style="font-size:1.2rem; font-weight:bold; margin-left:0.5rem;">' + bayNameOnlyNumber + '번</span>'
    	                	+ '<span style="color:green; font-size:1.2rem; margin-left:1.2rem;">' + bookingStatusText + '</span>' 
    	                : '<span style="font-size:1.3rem; font-weight:bold; margin-left:0.6rem;"> </span>'
       	                 ...
       	                 var listItem = 
       	                  '<div class="list__use">' +
       	                      '<div>' +
       	                          '<p><span style="width: 1.5rem;">' + rowNum + '.</span> <span style="width: 8.5rem;">' + timeParts.dateStr + '</span>' + '<span style="width: 3.5rem;">' 
       	                          + timeParts.startTimeStr + '</span>' + ' ~  <span style="width: 3.5rem;">'
       	                          + timeParts.endTimeStr + '</span>' 
       	                          + bayDisplay +                      
       	                          '</p>' +
       	                      '</div>' +
       	                  '</div>';
